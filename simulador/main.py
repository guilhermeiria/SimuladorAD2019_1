...
class Simulacao(object):
    ...
    def adicionarClienteNaFila(self, cliente):
        ...
        self.areaClientesFilaEsperaPorIntervalTempo += self.qntClientesFilaDeEspera * (
                    self.tempoAtual - self.instanteUltimoEvento)
    ...
    def executarSim(self):
        ...
        print
        "tempo de Simulacao: ", datetime.now() - inicioSim
        print
        "Ver.: 0.4.4"
        print
        "M/M/1: ", not self.modoDeExec
        print
        "Mu: ", self.mu
        print
        "Lambda: ", self.lambd
        print
        "FCFS: ", self.disciplinaAtendimentoFCFS
        print
        "Conteudo da fila"
        ...
        print
        "Total de Clientes atendidos: ", self.qntClientesAtendidos
        print
        "E[W]analitico = ", self.rho / (self.mu * (1 - self.rho))
        print
        "E[W]analtico rho estimado = ", self.esperanca_W_analitico(self.servidor.utilizacaoReal(self.tempoAtual),
                                                                   self.mu)
        print
        "E[W] simples: ", self.somaTempoFilaEspera / self.qntClientesAtendidos
        print
        "E[W] pela Calculadora: ", self.calculadoraAmostraWq.get_media()
        print
        "E[W] pela area calculada: ", self.areaClientesFilaEsperaPorIntervalTempo / self.qntClientesAtendidos
        print
        "V(W) pela Calculadora: ", self.calculadoraAmostraWq.get_variancia()
        print
        print
        "Media de Clientes na Fila de Espera: ", self.areaClientesFilaEsperaPorIntervalTempo / self.tempoAtual
        print
        "Media de Clientes na Fila de Espera analit. Teorico: ", self.lambd * (self.rho / (self.mu * (1 - self.rho)))
        print
        "Media de Clientes na Fila de Espera analit. Dados simulacao: ", self.esperanca_Nq_analitico(self.lambd,
                                                                                                     self.calculadoraAmostraWq.get_media())
        print
        "Utilizacao estimada do Servidor: ", self.servidor.utilizacaoReal(self.tempoAtual)
